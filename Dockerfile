...
# init
RUN apt-get update && apt-get -y upgrade && apt-get install -y --no-install-recommends openssl \
    unzip gdal-bin wget curl openjdk-11-jdk && apt clean && rm -rf /var/cache/apt/* && rm -rf /var/lib/apt/lists/*
